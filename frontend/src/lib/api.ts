...
export async function saveProject(digestData: Record<string, unknown>, graphData: Record<string, unknown>, pdfContent?: string): Promise<ProjectSaveResponse> {
...
export async function updateConceptNotes(
  projectId: string,
  conceptName: string,
  notes: string
): Promise<{ success: boolean; message: string }> {
  const response = await fetch(`${API_BASE_URL}/project/${projectId}/concept-notes`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  if (!response.ok) {
    throw new Error(`Failed to update concept notes: ${response.statusText}`);
  }
...
export async function updateConceptConfidence(
  ...
  const response = await fetch(`${API_BASE_URL}/project/${projectId}/concept-confidence`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  if (!response.ok) {
    throw new Error(`Failed to update concept confidence: ${response.statusText}`);
  }
...
export async function processPDF(file: File, onProgress?: (stage: string, progress: number) => void): Promise<string> {
  try {
    ...
    const saveData = await saveProject(digestData.digest_data, relationshipsData.graph_data, extractData.text);

    // Step 5: Store PDF in sessionStorage for quick access
    console.log("📄 Storing PDF in sessionStorage");
    console.log("   projectId:", saveData.project_id);
    console.log("   PDF length:", extractData.text.length);
    const storageKey = `pdf_${saveData.project_id}`;
    sessionStorage.setItem(storageKey, extractData.text);
    console.log("✅ PDF stored with key:", storageKey);
    console.log("✅ Verify - retrieve:", sessionStorage.getItem(storageKey)?.length, "chars");
