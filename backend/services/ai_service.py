...
async def generate_relationships(structured_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Add notes field to each node for user annotations
...
async def generate_concept_definition(concept_name: str, project_id: str, pdf_content: str = None) -> dict:
    """
    Generate a detailed definition of a concept from the uploaded PDF content
    Extracts actual text from PDF for real definitions
    """
    try:
        import re
        
        # Use provided PDF content, or try to get from database
        if not pdf_content:
            from services.db_service import get_project_pdf_content
            pdf_content = await get_project_pdf_content(project_id)
        
        if not pdf_content:
            return {
                "definition": f"Definition not found in uploaded materials.",
                "success": False,
                "error": "PDF content not available"
            }
        
        # DEBUG: Log what we're searching for
        print(f"🔍 Searching for concept: '{concept_name}' in PDF ({len(pdf_content)} chars)")
        ...
        idx = pdf_lower.find(concept_lower)
        if idx != -1:
            print(f"✅ Found exact match at position {idx}")
            # Extract surrounding context (500 chars before and after)
            start = max(0, idx - 200)
            end = min(len(pdf_content), idx + len(concept_name) + 500)
            context = pdf_content[start:end].strip()
            context = re.sub(r'\s+', ' ', context)
            
            if len(context) > 20:
                print(f"✅ Returning context: {context[:100]}...")
                return {
                    "definition": context[:800],
                    "success": True
                }
        
        # Strategy 2: Search for concept words (partial match)
        words = concept_name.lower().split()
        if len(words) > 0:
            # Search for the first/main word
            main_word = words[0]
            idx = pdf_lower.find(main_word)
            if idx != -1 and len(main_word) > 2:  # Only if meaningful word
                print(f"✅ Found partial match for '{main_word}' at position {idx}")
                # Extract surrounding context
                start = max(0, idx - 200)
                end = min(len(pdf_content), idx + 300)
                context = pdf_content[start:end].strip()
                context = re.sub(r'\s+', ' ', context)
                
                if len(context) > 20:
                    print(f"✅ Returning partial match context: {context[:100]}...")
                    return {
                        "definition": context[:800],
                        "success": True,
                        "note": "partial match"
        ...
        # Strategy 3: Return first 1000 chars of PDF as fallback
        print(f"⚠️ No match found, returning PDF excerpt as fallback")
        fallback = pdf_content[:1000].strip()
        ...
        if len(fallback) > 20:
            return {
                "definition": fallback,
                "success": True,
                "note": "fallback - PDF excerpt"
            }
        
        return {
            "definition": f"Could not extract definition for '{concept_name}' from PDF.",
            "success": False,
            "error": "Could not find matching content in PDF"
        }
